...
int battery_percent(void)
{
        ...
        FileGetInt(BATTERY_SYSFS_PATH "capacity", &capacity) < 0)
    {
        ...
        {
            if (FileGetInt(BATTERY_SYSFS_PATH "charge_full", &full))
                return -1;

            if (FileGetInt(BATTERY_SYSFS_PATH "charge_now", &now))
                return -1;
...
int battery_voltage(void)
{
    ...
    if (g_file_test(BATTERY_SYSFS_PATH "voltage_now", G_FILE_TEST_EXISTS) &&
        FileGetInt(BATTERY_SYSFS_PATH "voltage_now", &voltage))
        return -1;
